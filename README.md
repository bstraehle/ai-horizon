...
## Tree‑Shaking & Modular Consumption

The codebase is now structured to enable aggressive dead‑code elimination when you embed portions of the logic into another bundle or future modular builds:

- No default exports: all modules use named exports (`{ LeaderboardManager }`, `{ normalize }`, etc.) so bundlers can drop unused symbols precisely.
- `package.json` includes `"sideEffects": false` indicating that importing a module has no required runtime side effects (modules only define classes/functions/constants). If you later introduce a file with required side effects (e.g. a polyfill or CSS import), either remove the flag or list exceptions (e.g. `"sideEffects": ["js/polyfills.js"]`).
- Pure construction hints: Object pool instantiations are annotated with `/* @__PURE__ */` to help advanced minifiers remove them if a future build gates features behind conditions.
- Deep‑frozen config (`CONFIG`) is a plain object; property access patterns are friendly to minifiers (no dynamic string indexing for critical paths).

### Consuming Individual Utilities

Example: only need the seeded RNG and ObjectPool in another project?
...
Bundlers (esbuild, Rollup, webpack, Vite) will ignore unrelated managers/entities.

### Splitting the Game Further (Optional)

If you plan progressive loading (e.g. menu shell first, gameplay later):

1. Move heavy manager/entity groups (asteroids, explosions) into `./js/chunks/` and lazy import them in `game.js` when the player presses Start.
2. Keep only minimal bootstrap (canvas init, input binding, menu UI) in the initial bundle.
3. Guard dynamic imports with feature detection if needed.

### When Tree‑Shaking Might Not Remove Code

- Dynamic `import()` with template strings that can’t be statically analyzed.
- Reflective property access (e.g. `object[methodName]()` with user input) on exported objects.
- Re‑export \* namespaces (not currently used here) can hide symbol usage detail.

### Future Enhancements

- Split `constants.js` into domain modules (`config/game.js`, `config/spawn.js`, `config/ui.js`) to allow importing only subsets.
- Introduce optional build target that excludes the remote leaderboard (omit adapters + repository) for fully offline mode, shrinking bundle size.
- Provide a `js/entry/minimal.js` that wires only core loop & rendering for sandboxing experiments.
